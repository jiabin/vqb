...
JCSDL.Loader.addComponent(function($, undefined) {
	...
	this.GUI.prototype = {
		...
		createFilterRow : function(filter) {
            ...
            if (targetInfo.icon == undefined) {
                targetInfo.icon = 'no-icon fa fa-question';
            }

			var $target = this.getTemplate('filterTarget')
					.addClass('target-' + filter.target)
					.html('<span class="font-icon '+targetInfo.icon+'"></span>' + targetInfo.name);
			...
			$.each(filter.fieldPath, function(i, field) {
                ...
                if (fieldInfo.icon == undefined) {
                    fieldInfo.icon = 'no-icon fa fa-question';
                }
                
				var $field = self.getTemplate('filterField')
						// .addClass('icon-' + self.getIconForField(field, fieldInfo))
						.html('<span class="font-icon '+fieldInfo.icon+'"></span>' + fieldInfo.name);
